...
def text_gradient_angle(teks, list_warna, sudut_derajat=45):
  ...
  for y, baris in enumerate(baris_list):
    ...
    for x, karakter in enumerate(baris):
      ...
      posisi_global = max(0.0, min(1.0, (posisi_global + 1) / 2 if sudut_derajat != 0 and sudut_derajat != 90 else posisi_global))
      if sudut_derajat == 45: 
    ...
    if not dalam_menu_kalkulasi:
      with stdout_lock:
        ...
        if 5 <= jam < 12:
          ...
          color_code = AnsiColors.LIGHT_BLUE
        elif 17 <= jam < 19:
          greet = "Good Evening 🌆"
          color_code = AnsiColors.LIGHT_PURPLE
        else:
          greet = "Good Night 🌃"
