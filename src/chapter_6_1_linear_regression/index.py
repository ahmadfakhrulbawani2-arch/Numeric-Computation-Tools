...
def write_result():
    try:
        with open(file=OUT_PATH, mode="a", encoding="utf-8") as file:
            ...
            sys.stdout.write("\n\n\033[4A")
            sys.stdout.flush()
            print(f"\nFile saved successfully in {OUT_PATH}")
            print("Going back to menu in 2 seconds...")
            print()
            time.sleep(2)
    except FileNotFoundError:
        log_activities(f"File not found in {PROG_NAME}. Failed to write")
    ...
    try:
        with open(file=IN_PATH, mode="r", encoding="utf-8") as file:
            ...
            try:
                ...
                raise ValueError("File CSV harus memiliki kolom 'x' dan 'y' (case insensitive)")
        ...
        log_activities(f"File not found in {PROG_NAME}.")
        raise FileNotFoundError(f"File/Path not found")
