...
|_|   \___/|_|\__, |_| |_|\___/|_| |_| |_|_|\__,_|_|
 ____         |___/
|  _ \ ___  ____ ____ ___  _______(_) ___  _ __     
...
def reg_processing(raw_data: Csv_Data):
    ...
    if total_data < 2:
        print(
            f"{AnsiColors.RED}Data terlalu sedikit untuk melakukan regresi! Minimal butuh 2 pasang data.{AnsiColors.RESET}"
        )
        return
    ...
    while True:
        ...
        if key == "up":
            ...
            match curr_select:
                ...
                case 4:
                    clear_screen()
                    delete_output(OUT_PATH, PROG_NAME)
                    menu = "Misc"
                case last_idx:
                    style.stop_jam.set()
                    clear_screen()
