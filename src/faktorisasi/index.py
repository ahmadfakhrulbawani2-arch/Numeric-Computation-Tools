...
def calc_error(old_vals, new_vals) -> float:
    return max(abs(n - o) for o, n in zip(old_vals, new_vals))


# DEGREE 3
def lin_deg_3(eq: List[float], acc: float, max_iter: int, csv_file):
    ...
    for i in range(1, max_iter + 1):
        ...
        print(
            f"[{now}] Iter {i:3d} |  a0={a0:.2f}  b1={b1:.2f}  b0={b0:.2f}  %Ea={err:.2f}"
        )
...
# DEGREE 4
def lin_deg_4(eq: List[float], acc: float, max_iter: int, csv_file):
    ...
    for i in range(1, max_iter + 1):
        ...
        now = datetime.now()
        print(
            f"[{now}] Iter {i:3d} |  a0={a0:.2f}  a1={a1:.2f}  b0={b0:.2f}  b1={b1:.2f}  %Ea={err:.2f}"
        )
...
# DEGREE 5
def lin_deg_5(eq: List[float], acc: float, max_iter: int, csv_file):
    ...
    for i in range(1, max_iter + 1):
        ...
        now = datetime.now()
        print(
            f"[{now}] Iter {i:3d} |  a0={a0:.2f}  b1={b1:.2f}  b0={b0:.2f}  c1={c1:.2f}  c0={c0:.2f}  %Ea={err:.2f}"
        )
...
def lin_method_driver(eq: List[float], acc: float, max_iter: int, degree: int):

    csv_file = open(f"./out/{DIR_NAME}/iterations.csv", "w")
    ...
    match degree:
        ...
        case 3:
            roots = lin_deg_3(eq, acc, max_iter, csv_file)

        case 4:
            roots = lin_deg_4(eq, acc, max_iter, csv_file)

        case 5:
            roots = lin_deg_5(eq, acc, max_iter, csv_file)
